...
## Current Status — Phase 2.5, Slice 2 (Reliability & Observability) done

- **Stage:** Phase 1 (Milestones 1–4c) is complete and tagged `phase-1-complete` — auth, broker connection/sync, portfolio analytics, Public Investor Library, discovery feed, follows, and the full frontend. Phase 2 (Milestones 5–8) is complete: Milestone 5 (Health Metrics) resolved ADR-008 by adding portfolio volatility (ADR-024) and the `GET /portfolios/:id/history` endpoint + chart; Milestone 6 (Portfolio Comparison) added `GET /portfolios/compare?ids=a,b` (ADR-026/027); Milestone 7 (Strategy Categorization) added rules-based auto-tagging for 3 of 8 fixed categories (ADR-028); Milestone 8 (Dhan broker integration) is built and verified but not yet merged — kept on its own branch pending explicit commit authorization, per this project's git-safety practice. Phase 2.5 (Production Hardening) is underway, sequenced as four slices: Security & API (ADR-029–033) and Reliability & Observability (done this entry — see below), then Performance and Deployment. See [roadmap.md](./roadmap.md) "Phase 2.5" for the full sequencing and [changelog.md](./changelog.md) for what shipped in each slice.
- **This entry (Reliability & Observability):** the sync pipeline now retries a rate-limited broker call with backoff (safe before any DB write happens) but never retries an expired token or an unexpected error (ADR-034); every sync failure path — including one that was previously completely silent — consistently updates connection status, writes a structured log line, and writes an audit event; the daily scheduled sync no longer permanently drops a connection after one transient failure; a worker crash mid-sync no longer silently loses the job (`task_acks_late`); structured JSON logging with per-request correlation ids exists for the first time (previously there was exactly one logging call in the whole backend); `GET /health`/`GET /health/ready` exist for the first time. External alerting/APM was deliberately left out of scope this slice — deferred to Deployment, once a real hosting decision exists to tie it to.
...
- **Next step:** Phase 2.5, Slice 3 — Performance hardening (N+1 query audit, sync latency, caching opportunities) — see [roadmap.md](./roadmap.md) and [decisions.md](./decisions.md).
