...
def run_sync(broker_connection_id: str, is_final_attempt: bool = True) -> None:
    ...
    is_final_attempt (ADR-034): set by the Celery task wrapper
    (app/tasks/sync.py), the only caller that knows whether Celery's
    autoretry_for has another attempt left for a rate-limit failure. On a
    non-final attempt, a rate-limit error re-raises immediately (so Celery
    retries it) without touching connection.status or audit_logs —
    marking the connection "error" for a transient blip that resolves on
    retry would flip status back and forth and leave a spurious audit
    row for what turns out to be a successful sync. Direct callers (tests,
    or any caller with no retry wrapper) default to True, since without a
    bound Celery task there is no next attempt to wait for.
    """
    ...
    try:
        ...
        current_app.logger.warning(
            "Sync failed: token expired for broker_connection_id=%s", connection.id
        )
        ...
        # Safe to re-raise regardless of is_final_attempt: nothing has been
        # written to the DB yet, so a Celery retry re-runs this whole
        # function from scratch (ADR-034).
        if is_final_attempt:
            connection.status = "error"
            db.session.commit()
            current_app.logger.error(
                "Sync failed after exhausting retries (rate limit): "
                "broker_connection_id=%s reason=%s",
                connection.id,
                exc,
            )
            audit_service.log_event(
            ...
            current_app.logger.warning(
                "Sync rate-limited, retrying: broker_connection_id=%s reason=%s",
                connection.id,
                exc,
            )
        raise
        ...
        db.session.commit()
        current_app.logger.error(
            "Sync failed (API error): broker_connection_id=%s reason=%s",
            connection.id,
            exc,
        )
        audit_service.log_event(
    ...
    try:
        ...
        # Anything past a successful fetch_holdings that wasn't already
        # handled above (ADR-034) — a bug, a DB constraint violation, an
        # unexpected data shape. Roll back first: the session may already
        # be in a failed-flush state, and committing without rolling back
        # first can itself raise, masking the real error and silently
        # skipping the very status/audit write this block exists for.
        # Re-raised (never retried — nothing in autoretry_for matches a
        # bare Exception) so Celery's own task state honestly reports
        # FAILURE, and so this failure class stops being invisible to
        # both the audit trail and connection.status the way it was before.
        db.session.rollback()
        connection.status = "error"
        db.session.commit()
        current_app.logger.exception(
            "Sync failed unexpectedly: broker_connection_id=%s", connection.id
        )
        audit_service.log_event(
            connection.user_id,
            "error",
            {"broker_connection_id": str(connection.id), "reason": "unexpected_error"},
        )
        raise

    # Best-effort only, deliberately outside the try/except above: the sync
    # itself already committed successfully by this point, so a failure in
    # cache invalidation or the "sync" audit log shouldn't roll connection
    # back to "error" — the portfolio data is genuinely current, only a
    # peripheral side effect failed.
    try:
        ...
        current_app.logger.exception(
            "Failed to invalidate discovery cache after sync: broker_connection_id=%s",
            connection.id,
        )
    ...
    try:
        ...
        current_app.logger.exception(
            "Failed to write sync audit log: broker_connection_id=%s", connection.id
        )
